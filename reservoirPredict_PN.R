...
for (i in 1:nloops){
    ...
    # Summarize blast hits for training set
    ...
    set<-c("Reservoir",bp)
    f1_train<-f1_train[,c(set)] # this is the full training dataset with blast probabilities only
    ...
    write.csv(norm_cm,file=paste("Reservoir_PN_CM",i,".csv"))
    ...
    # Clean up
    h2o.rm("gbm_grid")
    rm(oBlast,testBlast,allBlast,rBlast,optBlast,trainSeqs,testSeqs,optSeqs,gbm_grid,best_gbm,train,test,opt,df2,optims)
}
...
write.csv(vimps,file="Reservoir_PN_FI.csv",row.names = T)
write.csv(accs,file="Reservoir_PN_out.csv",row.names=F)
