...
f_v2<-f_v[,c("Virus.name","Vector.borne","Viral.group","Genbank.accession")] 
f_v2$response<-factor(f_v2$Vector.borne)
...
for (i in 1:nloops){
  # 70:15:15 stratified split 
    ...
    #h2o.saveModel(best_gbm,path =paste("h2o_ab_PN_model",i))
    ...
    write.csv(norm_cm,file=paste("h2o_ab_PN_CM",i,".csv"))
...
write.csv(vimps,file="ArthropodBorne_PN_FI.csv",row.names = T)
write.csv(accs,file="ArthropodBorne_PN_out.csv",row.names=F)
write.csv(test.record,file="ArthropodBorne_PN_TestSets.csv",row.names=F)
